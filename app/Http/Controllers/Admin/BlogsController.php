...
class BlogsController extends Controller
{
    ...
    public function admin_store(Request $request)
    {
        ...
        if($blog)
        {
            ...
            $result = $this->__imageSave($request, $blog, 'BlogMeta');
           
            if(count($result)>0){
                foreach ($result as $Id => $imageFilename) {
                    $record = Blog::find($blog->id);
                    if ($record) {
                        $record->featured_id = $Id;
                        $record->featured_image = $imageFilename;
                        $record->update();
                    }
                }
            }
        ...
        foreach ($filedata as $imgKey => $imgValue) {
            ...
            if (is_array($imgValue['value'])) {
                ...
                foreach ($imgValue['value'] as $image) {
                    ...
                    $response=Blog::find($imgKey)->addMedia(storage_path('app/uploads/configuration-images/' . $fileFullName))
                ...
                // Add the media to the 'images' collection
                $response=Blog::find($imgKey)->addMedia(storage_path('app/uploads/configuration-images/' . $fileName))
                    ->toMediaCollection('images');
            }
