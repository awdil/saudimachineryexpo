...
class ConfigurationsController extends Controller
{
    ...
    public function admin_prefix(Request $request, $prefix = NULL) { 
        ...
        if($request->isMethod('post')) {
            ...
            if($request->has('Configuration')) {
                ...
                $fileNameArr = (isset($fileNameArrRes['fileNames']))?$fileNameArrRes['fileNames']:'';
                
                $configurationResponse = (!empty($fileNameArrRes['configurationResponse']))?$fileNameArrRes['configurationResponse']:'';
    ...
    **/
    ...
    public function removePublicImage($id){
        $media = Media::findOrFail($id);
        if($media){
            $media->delete();
        }
    ...
    public function removePublicMediaByModel($source, $model, $media_id){
        if($source== 'exhibition' || $source== 'exhibitions') {
            $modelInstance = Exhibition::findOrFail($model);
        }elseif($source== 'catalogue' || $source== 'catalogues') {
            $modelInstance = Catalogue::findOrFail($model);
        }elseif($source== 'product'|| $source== 'products') {
            $modelInstance = Product::findOrFail($model);
        }elseif($source== 'page'|| $source== 'pages') {
            $modelInstance = Page::findOrFail($model);
        }elseif($source== 'blog'|| $source== 'bloges') {
            $modelInstance = Blog::findOrFail($model);
        }else{
            abort(404);
        }
        $mediadata = Media::findOrFail($media_id);
        if($mediadata){
            $mediadata->delete();
            if($modelInstance){
                $modelInstance->update(['featured_id' => 0]);
            }
        }
    }
