						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XFeaturedImage {{ $screenOption['FeaturedImage']['visibility'] ? '' : 'd-none' }}" id="accordion-feature-image">
							...
							<div class="accordion__body p-4 collapse show" id="with-feature-image" data-bs-parent="#accordion-feature-image">
								<div class="featured-img-preview img-parent-box">
									...
									<?php  
									$routeName = 'public-media.PublicMediaImagePreview';
									$routeParameters = ['model' => 'blog', 'id' => $blog->id];
									$is_deleted = DzHelper::checkRouteStatus($routeName, $routeParameters);
									?>
										<!-- <img src="{{ $ximage_url }}" class="avatar img-for-onchange"  alt="{{ __('Image') }}" title="{{ __('Image') }}">  -->
										<!-- <img src="{{ route('public-media-file.PublicImagePreviewByMedia', ['model' => 'blog', 'id' => $blog->id, 'media' => $blog->featured_id]) }}" alt="{{ $blog->title }}"> -->
										@if($is_deleted)
										<img src="{{ asset('images/noimage.jpg') }}" class="avatar img-for-onchange"  alt="{{ __('Image') }}" width="100px" height="100px" title="{{ __('Image') }}"> 
										@else
										<!-- <img src="{{ route('public-media.PublicMediaImagePreview', ['model' => 'blog', 'id' => $blog->id]) }}" alt="{{ $blog->title }}"> -->
										<img src="{{ route('public-media-file.PublicImagePreviewByMedia', ['model' => 'blog', 'id' => $blog->id, 'media' => $blog->featured_id]) }}" alt="{{ $blog->title }}">
										@endif
									...
									<div class="d-flex align-items-center">
										@if(!empty($blog->featured_id) && $is_deleted!=1)
										<!-- <a href="javascript:void(0);" rdx-link="{{ route('remove-public-file-by-id.removePublicImage', $blog->featured_id) }}" class="rdxUpdateAjax btn btn-primary btn-xs rounded-0 me-2" rdx-delete-box="RemoveItemImg_{{ $blog->id }}">{{ __('Remove') }}</a> -->
										<a href="javascript:void(0);" rdx-link="{{ route('remove-public-file-by-model.removePublicMediaByModel', ['source' => 'blog', 'model' => $blog,'id' => $blog->featured_id]) }}" class="rdxUpdateAjax btn btn-primary btn-xs rounded-0 me-2" rdx-delete-box="RemoveItemImg_{{ $blog->id }}">{{ __('Remove') }}</a>
										@endif
										<div class="form-file">
									...
									<input type="hidden" name="data[BlogMeta][0][meta_id]" value="{{ isset($blog->featured_id) ? $blog->featured_id : 0 }}">
									<input type="hidden" name="data[BlogMeta][0][old_value]" value="{{ isset($blog->featured_id) ? $blog->featured_id : '' }}">
							   	</div>
