	...
	<div class="row page-titles mx-0">
		<div class="col-sm-6 p-0">
			<div class="welcome-text">
				<h4>{{ __('Catalogue') }}</h4>
				<span>{{ __('Edit Catalogue') }}</span>
			</div>
		</div>
		<div class="col-sm-6 p-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="{{ route('exhibitions.admin.index') }}">{{ __('Catalogue') }}</a></li>
				<li class="breadcrumb-item active"><a href="javascript:void(0)">{{ __('Edit Catalogue') }}</a></li>
			</ol>
		</div>
	</div>

	<form action="{{ route('catalogues.admin.update', $catalogue->id) }}" method="post" enctype="multipart/form-data">
		@csrf
		<div class="row">
			<div class="col-md-8">
				<div class="row">
							...
							<div class="card-body p-4">
								<div class="row">
									<div class="form-group col-md-12">
										...
										<input type="text" name="data[Catalogue][title]" class="form-control" id="BlogTitle" placeholder="{{ __('Title') }}" value="{{ old('data.Exhibition.title', $catalogue->title) }}">
									...
									<div class="form-group col-md-12">
										...
										<a href="{!! DzHelper::laraBlogLink($catalogue->id) !!}">
											{{ url('/') }}/<span class='font-green permalinkSlugSpan'>{{ $catalogue->slug }}</span>
										</a>
										<div class="editPermalinkContainer">
											<input type="text" name="data[Catalogue][editslug]" id="BlogEditSlug" class="form-control" value="{{ $catalogue->slug }}">
									...
									<div class="form-group col-md-12">
										<textarea name="data[Catalogue][content]" class="form-control W3cmsCkeditor h-auto" id="BlogContent" rows="10">{{ old('data.Exhibition.content', $catalogue->content) }}</textarea>
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XExcerpt {{ $screenOption['Excerpt']['visibility'] ? '' : 'd-none' }}" id="accordion-excerpt">
							...
							<div class="accordion__body p-4 collapse show" id="with-excerpt" data-bs-parent="#accordion-excerpt">
								<div class="form-group">
									...
									<textarea name="data[Catalogue][excerpt]" class="form-control mb-1" id="ContentExcerpt" rows="5">{{ old('data.Exhibition.excerpt', $catalogue->excerpt) }}</textarea>
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XCustomFields {{ $screenOption['CustomFields']['visibility'] ? '' : 'd-none' }}" id="accordion-custom-fields">
							...
							<div class="accordion__body p-4 collapse show" id="with-custom-fields" data-bs-parent="#accordion-custom-fields">
								<div id="AppendContainer">
									@php
										$count = 1;
										$custom_fields = old('data.ExhibitionMeta') ? old('data.ExhibitionMeta') : $catalogue->blog_meta;
									@endphp
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XDiscussion {{ $screenOption['Discussion']['visibility'] ? '' : 'd-none' }}" id="accordion-discussion">
							...
							<div class="accordion__body p-4 collapse show" id="with-discussion" data-bs-parent="#accordion-discussion">
								<div class="form-check mb-2">
									...
									<input type="checkbox" name="data[Catalogue][comment]" class="form-check-input" id="ContentComment" value="1" {{ old('data.Exhibition.comment', $catalogue->comment) == '1' ? 'checked' : '' }}>
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XSlug {{ $screenOption['Slug']['visibility'] ? '' : 'd-none' }}" id="accordion-slug">
							...
							<div class="accordion__body p-4 collapse show" id="with-slug" data-bs-parent="#accordion-slug">
								<div class="form-group">
									...
									<input type="text" name="data[Catalogue][slug]" class="form-control slug" id="slug" value="{{ old('data.Exhibition.slug', $catalogue->slug) }}" disabled="disabled">
								...
								<div class="form-group">
									...
									<select name="data[Catalogue][user_id]" class="default-select form-control" id="ContentUserId">
										@forelse($users as $user)
											<option value="{{ $user->id }}" {{ old('data.Exhibition.user_id', $catalogue->user_id) == $user->id ? 'selected="selected"' : '' }}>{{ $user->full_name }}</option>
										@empty
										@endforelse
									</select>
								</div>
							...
							<div class="accordion__body p-4 collapse show" id="with-seo" data-bs-parent="#accordion-seo">
								<div class="row">
									<div class="col-md-12 form-group">
										...
										<input type="text" name="data[BlogSeo][page_title]" class="form-control" id="ContentSeoBlogTitle" placeholder="{{ __('Blog Title') }}" maxlength="255" value="{{ old('data.BlogSeo.page_title', optional($catalogue->blog_seo)->page_title) }}">
									</div>
									<div class=" form-group col-md-6">
										<label for="ContentSeoMetaKeywords">{{ __('Keywords') }}</label>
										<input type="text" name="data[BlogSeo][meta_keywords]" class="form-control" id="ContentSeoMetaKeywords" placeholder="{{ __('Enter meta keywords') }}" maxlength="255" value="{{ old('data.BlogSeo.meta_keywords', optional($catalogue->blog_seo)->meta_keywords) }}">
									</div>
									<div class="form-group col-md-6">
										<label for="ContentSeoMetaDescriptions">{{ __('Descriptions') }}</label>
										<textarea name="data[BlogSeo][meta_descriptions]" class="form-control" id="ContentSeoMetaDescriptions" rows="5" placeholder="{{ __('Enter meta descriptions') }}">{{ old('data.BlogSeo.meta_descriptions', optional($catalogue->blog_seo)->meta_descriptions) }}</textarea>
									</div>
								</div>
							</div>
						</div>
			...
			<div class="col-md-4">
				<div class="row">
								...
								<div class="row">
									<div class="col-md-12 form-group">
										...
										<select name="data[Catalogue][status]" id="Status" class="default-select form-control">
											<option value="1" {{ old('data.Exhibition.status', $catalogue->status) == 1 ? 'selected="selected"' : '' }}>{{ __('Published') }}</option>
		                    				<option value="2" {{ old('data.Exhibition.status', $catalogue->status) == 2 ? 'selected="selected"' : '' }}>{{ __('Draft') }}</option>

		                    				@if($catalogue->status == 3)
		                    					<option value="3" {{ old('data.Exhibition.status', $catalogue->status) == 3 ? 'selected="selected"' : '' }}>{{ __('Trash') }}</option>
		                    				@endif
		                    				<option value="4" {{ old('data.Exhibition.status', $catalogue->status) == 4 ? 'selected="selected"' : '' }}>{{ __('Private') }}</option>
		                    				<option value="5" {{ old('data.Exhibition.status', $catalogue->status) == 5 ? 'selected="selected"' : '' }}>{{ __('Pending') }}</option>
										</select>
									</div>
									<div class="col-md-12 form-group">
										...
										<select name="data[Catalogue][visibility]" id="ContentVisibility" class="default-select form-control">
											<option value="Pu" {{ old('data.Exhibition.visibility', $catalogue->visibility) == 'Pu' ? 'selected="selected"' : '' }}>{{ __('Public') }}</option>
		                    				<option value="PP" {{ old('data.Exhibition.visibility', $catalogue->visibility) == 'PP' ? 'selected="selected"' : '' }}>{{ __('Password protected') }}</option>
		                    				<option value="Pr" {{ old('data.Exhibition.visibility', $catalogue->visibility) == 'Pr' ? 'selected="selected"' : '' }}>{{ __('Private') }}</option>
										</select>
									</div>
									<div class="col-md-12 form-group {{ old('visibility', $catalogue->visibility) == 'PP' ? '' : 'd-none' }}" id="PublicPasswordTextbox">
										<label for="ContentPassword">{{ __('Password') }}</label>
										<input type="password" name="data[Catalogue][password]" class="form-control" id="ContentPassword" placeholder="{{ __('Enter Password') }}" value="{{ old('data.Exhibition.password', $catalogue->password)}}" autocomplete="New-Password">
									</div>
									<div class="col-md-12 form-group" id="PublicPasswordTextbox">
										...
										<input type="text" name="data[Catalogue][publish_on]" class="datetimepicker form-control" id="PublishDateTimeTextbox" value="{{ $catalogue->publish_on ? old('data.Exhibition.publish_on', $catalogue->publish_on) : date('Y-m-d') }}">
									</div>
									<div class="col-md-12 form-group">
									...
									<div class="col-md-12 form-group">
										<label for=""><i class="fa fa-calendar"></i> {{ __('Exhibition') }}</label>
										<select name="data[Catalogue][exhibition_id]" id="year" class="default-select form-control">
											@foreach($exhibitions as $exhibition)
											<option value="{{ $exhibition->id }}" {{ old('year') == $year ? 'selected="selected"' : '' }}>{{ $exhibition->title }}</option>
											@endforeach
										</select>
									</div>
									<div class="col-md-12">
										...
										@if($catalogue->status != 3)
			                    			<a href="{{ route('blog.admin.admin_trash_status', $catalogue->id) }}" class="btn btn-danger">{{ __('Move to Trash') }}</a>
			                    		@endif
									</div>
								</div>
							</div>
					...
					<div class="col-md-12">
						@php
							$ximageMeta = $catalogue->feature_img;
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XFeaturedImage {{ $screenOption['FeaturedImage']['visibility'] ? '' : 'd-none' }}" id="accordion-feature-image">
							...
							<div class="accordion__body p-4 collapse show" id="with-feature-image" data-bs-parent="#accordion-feature-image">
								<div class="featured-img-preview img-parent-box">
									<div id="RemoveItemImg_{{ $catalogue->id }}">
										<!-- <img src="{{ route('exhibition.exhibitionPublicImagePreview', ['id' =>  $catalogue->id]) }}" class="avatar img-for-onchange"  alt="{{ __('Image') }}" title="{{ __('Image') }}">  -->
										
										<img src="{{ route('catalogue.catalougePublicImagePreview', ['id' =>  $catalogue->id]) }}" class="avatar img-for-onchange"  alt="{{ __('Image') }}" title="{{ __('Image') }}"> 

									</div>
									<div class="d-flex align-items-center">
										@if(!empty($ximageMeta->value))
										<a href="javascript:void(0);" rdx-link="{{ route('blog.admin.remove_feature_image', $catalogue->id) }}" class="rdxUpdateAjax btn btn-primary btn-xs rounded-0 me-2" rdx-delete-box="RemoveItemImg_{{ $catalogue->id }}">{{ __('Remove') }}</a>
										@endif
										<div class="form-file">
