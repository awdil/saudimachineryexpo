	...
	<div class="row page-titles mx-0 ">
		<div class="col-sm-6 p-0">
			<div class="welcome-text">
				...
				<span>{{ __('Add Exhibition') }}</span>
			</div>
		</div>
		<div class="col-sm-6 p-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="{{ route('blog.admin.index') }}">{{ __('Exhibitions') }}</a></li>
				<li class="breadcrumb-item active"><a href="javascript:void(0)">{{ __('Add Exhibition') }}</a></li>
			</ol>
		</div>
	</div>

	<form action="{{ route('exhibitions.admin.store') }}" method="post" enctype="multipart/form-data">
		@csrf
		<div class="row">
			<div class="col-md-8">
				<div class="row">
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								<h4 class="card-title">{{ __('Add Exhibition') }}</h4>
							</div>
							<div class="card-body p-4">
								<div class="row">
									<div class="form-group col-md-12">
										<label for="ExhibitionTitle">{{ __('Title') }}</label>
										<input type="text" name="data[Exhibition][title]" class="form-control" id="ExhibitionTitle" placeholder="{{ __('Title') }}" value="{{ old('data.Exhibition.title') }}">
									...
									<div class="form-group col-md-12 ">
											<textarea name="data[Exhibition][content]" class="form-control W3cmsCkeditor h-auto" id="ExhibitionContent" rows="10">{{ old('data.Exhibition.content') }}</textarea>
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XExcerpt {{ $screenOption['Excerpt']['visibility'] ? '' : 'd-none' }}" id="accordion-excerpt">
							...
							<div class="accordion__body p-4 collapse show" id="with-excerpt" data-bs-parent="#accordion-excerpt">
								<div class="form-group">
									...
									<textarea name="data[Exhibition][excerpt]" class="form-control" id="ContentExcerpt" rows="5">{{ old('data.Exhibition.excerpt') }}</textarea>
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XCustomFields {{ $screenOption['CustomFields']['visibility'] ? '' : 'd-none' }}" id="accordion-custom-fields">
							...
							<div class="accordion__body p-4 collapse show" id="with-custom-fields" data-bs-parent="#accordion-custom-fields">
								<div id="AppendContainer">
									@php
										$count = 1;
										$custom_fields = old('data.ExhibitionMeta');
									@endphp
											...
											@foreach($custom_fields as $custom_field)
												...
												<div class="row xrow">
													...
													<div class="col-md-6 form-group"> 
														...
														<textarea name="data[ExhibitionMeta][{{ $count }}][value]" id="ExhibitionMetaValue_{{ $count }}" class="form-control" rows="5">{{ isset($custom_field['value']) ? $custom_field['value'] : '' }}</textarea> 
													</div> 
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XDiscussion {{ $screenOption['Discussion']['visibility'] ? '' : 'd-none' }}" id="accordion-discussion">
							...
							<div class="accordion__body p-4 collapse show" id="with-discussion" data-bs-parent="#accordion-discussion">
								<div class="form-check mb-2">
									...
									<input type="checkbox" name="data[Exhibition][comment]" class="form-check-input" id="ContentComment" value="1" {{ old('data.Exhibition.comment') == 1 ? 'checked="checked"' : '' }}>
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XSlug {{ $screenOption['Slug']['visibility'] ? '' : 'd-none' }}" id="accordion-slug">
							...
							<div class="accordion__body p-4 collapse show" id="with-slug" data-bs-parent="#accordion-slug">
								<div class="form-group col-md-12">
									...
									<input type="text" name="data[Exhibition][slug]" class="form-control slug" id="slug" value="{{ old('data.Exhibition.slug') }}">
								</div>			
								...
								<div class="form-group">
									...
									<select name="data[Exhibition][user_id]" class="default-select form-control" id="ContentUserId">
										@forelse($users as $user)
											<option value="{{ $user->id }}" {{ old('data.Exhibition.user_id') == $user->id ? 'selected="selected"' : '' }}>{{ $user->full_name }}</option>
										@empty
										@endforelse
									</select>
								</div>
								...
								<div class="row">
									<div class="col-md-12 form-group">
										...
										<select name="data[Exhibition][status]" id="Status" class="default-select form-control">
											<option value="1" {{ old('data.Exhibition.status') == 1 ? 'selected="selected"' : '' }}>{{ __('Published') }}</option>
											<option value="2" {{ old('data.Exhibition.status') == 2 ? 'selected="selected"' : '' }}>{{ __('Draft') }}</option>
											<option value="4" {{ old('data.Exhibition.status') == 4 ? 'selected="selected"' : '' }}>{{ __('Private') }}</option>
											<option value="5" {{ old('data.Exhibition.status') == 5 ? 'selected="selected"' : '' }}>{{ __('Pending') }}</option>
										</select>
									</div>
									<div class="col-md-12 form-group">
										...
										<select name="data[Exhibition][visibility]" id="ContentVisibility" class="default-select form-control">
											<option value="Pu" {{ old('data.Exhibition.visibility') == 'Pu' ? 'selected="selected"' : '' }}>{{ __('Public') }}</option>
											<option value="PP" {{ old('data.Exhibition.visibility') == 'PP' ? 'selected="selected"' : '' }}>{{ __('Password protected') }}</option>
											<option value="Pr" {{ old('data.Exhibition.visibility') == 'Pr' ? 'selected="selected"' : '' }}>{{ __('Private') }}</option>
										</select>
									</div>
									<div class="col-md-12 form-group {{ old('data.Exhibition.visibility') == 'PP' ? '' : 'd-none' }}" id="PublicPasswordTextbox">
										<label for="ContentPassword">{{ __('Password') }}</label>
										<input type="password" name="data[Exhibition][password]" class="form-control" id="ContentPassword" placeholder="{{ __('Enter Password') }}" value="{{ old('data.Exhibition.password') }}" autocomplete="New-Password">
									</div>
					...
					<div class="col-md-12">
						<div class="card accordion accordion-rounded-stylish accordion-bordered XVideo {{ $screenOption['Video']['visibility'] ? '' : 'd-none' }}" id="accordion-video">
							...
							<div class="accordion__body p-4 collapse show" id="with-video" data-bs-parent="#accordion-video">
								...
								<input type="text" name="data[ExhibitionMeta][1][value]" class="form-control bootstrap-tagsinput" placeholder="{{ __('Youtube Video Link') }}" id="ExhibitionMeta1Value" value="{{ old('data.ExhibitionMeta.1.value') }}">
							</div>
						</div>
					</div>
				</div>
